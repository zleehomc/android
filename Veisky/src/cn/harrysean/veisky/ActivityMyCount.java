...
public class ActivityMyCount extends FinalActivity{
	@ViewInject(id = R.id.orderlise)
	ListView lv_order;
	...
	protected void setInList(JSONArray jArray) {
		...
		final SimpleAdapter listItemAdapter = new SimpleAdapter(this,
				list_order,// 数据源
				R.layout.myorderlist, new String[] { "number", "shopname" ,"price","gname","count","state"},
				new int[] { R.id.ordernumber,R.id.order_shopname,R.id.billprice,R.id.bill_goodsname,R.id.bill_goodsnum,R.id.state});

		// 添加并且显示
		lv_order.setAdapter(listItemAdapter);
